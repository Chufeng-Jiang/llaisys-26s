...
class Qwen2:
    ...
    def __init__(self, model_path, device: DeviceType = DeviceType.CPU):
        ...
        self.meta = LlaisysQwen2Meta(
            dtype=int(dtype),
            nlayer=int(self.config["num_hidden_layers"]),
            hs=hidden_size,
            nh=num_attention_heads,
            nkvh=int(self.config["num_key_value_heads"]),
            dh=head_dim,
            di=int(self.config["intermediate_size"]),
        ...
        self._device_ids = (ctypes.c_int * 1)(self.device_id)

        try:
            self._create_backend_model()

            self._safetensor_files = sorted(self.model_path.glob("*.safetensors"))
    ...
    def _create_backend_model(self):
        ...
        self._weights_pointer = LIB_LLAISYS.llaisysQwen2ModelWeights(self._model)

        if not self._weights_pointer:
            raise RuntimeError("Failed to retrieve Qwen2 weights.")

        self._weights = self._weights_pointer.contents
    ...
    def _build_tensor_manifest(self):
        ...
        for file_path in self._safetensor_files:
            header, header_length = self._read_safetensors_header(file_path)
    ...
    def _build_weight_targets(self):
        ...
        for layer_index in range(int(self.meta.nlayer)):
    ...
    def _create_tensor_from_mmap(self, mapped, tensor_name, tensor_info):
        ...
        shape_values = [int(value) for value in tensor_info["shape"]]

        if not shape_values:
            raise ValueError(f"Scalar tensor is not supported: {tensor_name}")

        relative_start, relative_end = tensor_info["data_offsets"]

        data_section_start = 8 + tensor_info["header_length"]

        absolute_start = data_section_start + relative_start

        absolute_end = data_section_start + relative_end

        actual_byte_count = absolute_end - absolute_start

        expected_byte_count = self._numel(shape_values) * element_size
        ...
        ShapeArray = ctypes.c_size_t * len(shape_values)
        ...
        if not tensor:
            raise RuntimeError(f"tensorCreate failed for {tensor_name}.")
    ...
    def generate(
        ...
        if not self._weights_loaded:
            raise RuntimeError("Qwen2 weights have not been loaded.")

        if not self._model:
            raise RuntimeError("Qwen2 backend model has been closed.")
        ...
        for _ in range(max_new_tokens):
            ...
            TokenArray = ctypes.c_int64 * len(pending_tokens)
            ...
            if next_token == int(self.meta.end_token):
                break

            # 后续调用只处理刚刚生成的一个 token。
            pending_tokens = [next_token]
