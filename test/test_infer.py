...
def load_hf_model(model_path=None, device_name="cpu"):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_path, torch_dtype=torch.bfloat16, device_map=torch_device(device_name), trust_remote_code=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", default="cpu", choices=["cpu", "nvidia", "metax"], type=str, help="Device used by LLAISYS."
    )
    ...
    if args.hf_device is not None:
        hf_device_name = args.hf_device
    elif args.device == "metax":
        hf_device_name = "cpu"
    else:
        hf_device_name = args.device
    ...
    tokenizer, model, model_path = load_hf_model(args.model, hf_device_name)

    start_time = time.time()

    tokens, output = hf_infer(
        args.prompt, tokenizer, model, max_new_tokens=args.max_steps, top_p=top_p, top_k=top_k, temperature=temperature
    )

    end_time = time.time()

    hf_elapsed = end_time - start_time

    del model

    gc.collect()

    if hf_device_name == "nvidia" and torch.cuda.is_available():
    ...
    llaisys_elapsed = end_time - start_time
