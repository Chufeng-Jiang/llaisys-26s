...
def test_ordering(shape, dtype_name, atol, rtol):
    a, a_ = random_tensor(shape, dtype_name, "nvidia")

    b, b_ = random_tensor(shape, dtype_name, "nvidia")

    _, tmp1_ = random_tensor(shape, dtype_name, "nvidia")

    _, tmp2_ = random_tensor(shape, dtype_name, "nvidia")

    _, out_ = random_tensor(shape, dtype_name, "nvidia")
    ...
    # Native CUDA:
    # tmp1 = a + b
    llaisys.Ops.add(tmp1_, a_, b_)

    # Triton:
    # tmp2 = tmp1 + b
    triton_add(tmp2_, tmp1_, b_)

    # Native CUDA:
    # out = tmp2 + b
    llaisys.Ops.add(out_, tmp2_, b_)
    ...
    assert check_equal(out_, expected, atol=atol, rtol=rtol)
...
if __name__ == "__main__":
    ...
    for round_idx in range(rounds):
        for shape, dtype_name, atol, rtol in test_cases:
            test_ordering(shape, dtype_name, atol, rtol)
