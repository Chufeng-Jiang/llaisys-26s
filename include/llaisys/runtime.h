...
__C {
    ...
    typedef void (*memcpy_async_api)(
        void *, const void *, size_t, llaisysMemcpyKind_t, llaisysStream_t);
    ...
    __export int llaisysSetContextRuntime(llaisysDeviceType_t device_type, int device_id);

    // Get the stream owned by the selected LLAISYS Runtime.
    //
    // This does not create a new stream. The returned stream is owned by
    // the LLAISYS Runtime and must not be destroyed by the caller.
    __export int llaisysGetContextStream(
        llaisysDeviceType_t device_type, int device_id, llaisysStream_t *stream);
}
