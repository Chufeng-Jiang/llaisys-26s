...
__C {
    LlaisysQwen2Model *llaisysQwen2ModelCreate(
        ...
        if (meta == nullptr) {
            std::cerr << "Qwen2: meta must not be null." << std::endl;
        ...
        try {
            ...
            std::cerr << "Qwen2 model creation failed: " << error.what() << std::endl;
    ...
    std::int64_t llaisysQwen2ModelInfer(
        LlaisysQwen2Model * model, std::int64_t *token_ids, std::size_t ntoken) {
        if (model == nullptr) {
            std::cerr << "Qwen2 inference failed: model is null." << std::endl;
        ...
        try {
            ...
            std::cerr << "Qwen2 inference failed: " << error.what() << std::endl;
