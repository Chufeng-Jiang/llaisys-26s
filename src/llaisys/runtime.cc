...
__C int
llaisysGetContextStream(llaisysDeviceType_t device_type, int device_id, llaisysStream_t *stream) {
    return llaisys::c_api::guard([&]() {
        if (stream == nullptr) {
            throw std::invalid_argument("llaisysGetContextStream: stream must not be null.");
        }

        auto &context = llaisys::core::context();

        context.setDevice(device_type, device_id);
...
__C const LlaisysRuntimeAPI *llaisysGetRuntimeAPI(llaisysDeviceType_t device_type) {
    return llaisys::c_api::guard_result<const LlaisysRuntimeAPI *>(
        [&]() { return llaisys::device::getRuntimeAPI(device_type); }, nullptr);
}