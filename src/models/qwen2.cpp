...
LlaisysQwen2Model::LlaisysQwen2Model(
	...
	: meta(model_meta),
	  device(model_device) {
	qwen2_validate_model_configuration(
		meta,
		model_device_ids,
		ndevice
	);
	...
	workspace =
		std::make_unique<Qwen2Workspace>(
			device,
			device_ids.front()
		);
...
std::int64_t LlaisysQwen2Model::infer(
	...
	qwen2_validate_inference_request(
		*this,
		token_ids,
		ntoken
	);
	...
	const Qwen2GlobalWeights global_weights =
		qwen2_global_weights(
			*this
		);
	...
	for (
		...
		// per-layer view.
		const Qwen2LayerWeights layer_weights =
			qwen2_layer_weights(
				*this,
				layer
			);
		...
		const Qwen2WorkspaceSlot output_slot =
			(layer % 2 == 0)
				? Qwen2WorkspaceSlot::Hidden1
				: Qwen2WorkspaceSlot::Hidden0;
	...
	return qwen2_predict_next_token(
		*this,
		hidden_states,
		sequence_length,
		global_weights
