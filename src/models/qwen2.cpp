...
llaisysTensor_t *allocate_layer_weights(
	std::size_t nlayer
) {
	return new llaisysTensor_t[nlayer]{};
}

llaisys::tensor_t require_tensor(
	llaisysTensor_t handle,
	const std::string &name
) {
	if (handle == nullptr) {
		throw std::runtime_error(
			"Qwen2 weight handle is null: " + name
		);
	}

	if (handle->tensor == nullptr) {
		throw std::runtime_error(
			"Qwen2 internal tensor is null: " + name
		);
	}
...
void validate_global_weights(
	const LlaisysQwen2Model &model
) {
	require_tensor(
		model.weights.in_embed,
		"model.embed_tokens.weight"
	);

	require_tensor(
		model.weights.out_embed,
		"lm_head.weight"
	);

	require_tensor(
		model.weights.out_norm_w,
		"model.norm.weight"
	);
}

void validate_layer_weight_arrays(
	const LlaisysQwen2Model &model
) {
	if (
		model.weights.attn_norm_w == nullptr
		|| model.weights.attn_q_w == nullptr
		|| model.weights.attn_q_b == nullptr
		|| model.weights.attn_k_w == nullptr
		|| model.weights.attn_k_b == nullptr
		|| model.weights.attn_v_w == nullptr
		|| model.weights.attn_v_b == nullptr
		|| model.weights.attn_o_w == nullptr
		|| model.weights.mlp_norm_w == nullptr
		|| model.weights.mlp_gate_w == nullptr
		|| model.weights.mlp_up_w == nullptr
		|| model.weights.mlp_down_w == nullptr
	) {
		throw std::runtime_error(
			"Qwen2 layer-weight arrays are not initialized."
		);
	}
}
...
LlaisysQwen2Model::LlaisysQwen2Model(
	...
	if (ndevice <= 0) {
		throw std::invalid_argument(
			"Qwen2: ndevice must be greater than zero."
		);
	}

	if (model_device_ids == nullptr) {
		throw std::invalid_argument(
			"Qwen2: device_ids must not be null."
		);
	}
	...
	if (ndevice != 1) {
		throw std::invalid_argument(
			"Qwen2: only one inference device is currently supported."
		);
	}

	if (meta.nlayer == 0) {
		throw std::invalid_argument(
			"Qwen2: number of layers must be greater than zero."
		);
	}

	if (meta.hs == 0) {
		throw std::invalid_argument(
			"Qwen2: hidden size must be greater than zero."
		);
	}

	if (meta.nh == 0) {
		throw std::invalid_argument(
			"Qwen2: attention head count must be greater than zero."
		);
	}

	if (meta.nkvh == 0) {
		throw std::invalid_argument(
			"Qwen2: KV head count must be greater than zero."
		);
	}

	if (meta.dh == 0) {
		throw std::invalid_argument(
			"Qwen2: head dimension must be greater than zero."
		);
	}
	...
	if (meta.di == 0) {
		throw std::invalid_argument(
			"Qwen2: intermediate size must be greater than zero."
		);
	}

	if (meta.maxseq == 0) {
		throw std::invalid_argument(
			"Qwen2: maximum sequence length must be greater than zero."
		);
	}

	if (meta.voc == 0) {
		throw std::invalid_argument(
			"Qwen2: vocabulary size must be greater than zero."
		);
	}

	device_ids.assign(
		model_device_ids,
		model_device_ids + ndevice
	);

	const std::size_t nlayer =
		meta.nlayer;

	weights.attn_norm_w =
		allocate_layer_weights(nlayer);
...
void LlaisysQwen2Model::ensure_cache_capacity(
	...
	if (new_capacity == 0) {
		new_capacity = std::min<std::size_t>(
			meta.maxseq,
			64
		);
	}
	...
	for (
		...
		auto new_key_cache = create_tensor(
			*this,
			{
				new_capacity,
				meta.nkvh,
		...
		auto new_value_cache = create_tensor(
			*this,
			{
				new_capacity,
				meta.nkvh,
		...
		if (
			...
			&& value_cache[layer] != nullptr
		) {
			auto old_key_prefix =
				key_cache[layer]->slice(
					0,
					0,
					cache_length
				);

			auto old_value_prefix =
				value_cache[layer]->slice(
					0,
					0,
					cache_length
				);
			...
			auto new_value_prefix =
				new_value_cache->slice(
					0,
					0,
					cache_length
				);
		...
		key_cache[layer] =
			std::move(new_key_cache);

		value_cache[layer] =
			std::move(new_value_cache);
	}
	
	std::cout
	<< "Qwen2 KV cache capacity: "
	<< cache_capacity
	<< " -> "
	<< new_capacity
	<< std::endl;
...
std::int64_t LlaisysQwen2Model::infer(
	const std::int64_t *token_ids,
	std::size_t ntoken
) {
	if (token_ids == nullptr) {
		throw std::invalid_argument(
			"Qwen2 inference token_ids must not be null."
		);
	}
	...
	if (cache_length > meta.maxseq) {
		throw std::runtime_error(
			"Qwen2 KV cache length is invalid."
		);
	}
	...
	const std::size_t previous_cache_length =
	cache_length;

	const std::size_t total_length =
		previous_cache_length + sequence_length;
	...
	const std::size_t attention_heads =
		meta.nh;
	...
	for (
		...
		position_values[index] =
			static_cast<std::int64_t>(
				previous_cache_length + index
			);
	}

	auto position_ids = create_tensor(
		*this,
		{sequence_length},
		LLAISYS_DTYPE_I64
	);
	 ...
	 * [S] × [V, H] -> [S, H]
	 */
	auto hidden_states = create_tensor(
		*this,
	...
	for (
		 ...
		 * Attention RMSNorm
		 */
		auto normalized_hidden = create_tensor(
			*this,
			{sequence_length, hidden_size},
			dtype
		);
		 ...
		 * RoPE
		 */
		auto rotated_query = create_tensor(
			*this,
			{
				sequence_length,
				attention_heads,
		...
		auto rotated_key = create_tensor(
			*this,
			{
				sequence_length,
				kv_heads,
		...
		*/
		...
		llaisys::ops::rearrange(
			key_cache_destination,
			rotated_key
		);

		llaisys::ops::rearrange(
			value_cache_destination,
			value_3d
		);
		...
		*/
		 ...
		 */
		auto attention_output_3d = create_tensor(
			*this,
			{
				sequence_length,
				attention_heads,
		...
		auto attention_output_2d =
			attention_output_3d->view(
				{
					sequence_length,
					hidden_size
				}
		 ...
		 * MLP RMSNorm.
		 */
		auto normalized_mlp = create_tensor(
			*this,
			{sequence_length, hidden_size},
			dtype
		);
		 ...
		 */
		auto gate = create_tensor(
			*this,
			{sequence_length, intermediate_size},
			dtype
		);

		auto up = create_tensor(
			*this,
			{sequence_length, intermediate_size},
			dtype
		);
	...
	*/
	 ...
	 * Shape: [1, H]
	 */
	auto last_hidden = hidden_states->slice(
		0,
		sequence_length - 1,
		sequence_length
	);
	...
	auto max_index = create_tensor(
		*this,
		{1},
		LLAISYS_DTYPE_I64
	);

	auto max_value = create_tensor(
		*this,
		{1},
		dtype
	);
