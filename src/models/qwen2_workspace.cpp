...
std::size_t Qwen2Workspace::checked_numel(
	...
	for (const std::size_t dimension : shape) {
		...
		if (
			...
			throw std::overflow_error(
				"Qwen2 workspace tensor size overflow."
			);
		}
...
std::size_t Qwen2Workspace::grow_capacity(
	std::size_t current,
	std::size_t required
) {
	if (required == 0) {
...
llaisys::tensor_t Qwen2Workspace::get(
	...
	if (index >= entries_.size()) {
		throw std::out_of_range(
			"Qwen2 workspace slot is out of range."
		);
	}
	...
	if (
		!entry.initialized
		|| dtype_changed
		|| entry.capacity_elements < required_elements
	) {
		const std::size_t new_capacity =
			dtype_changed
				? required_elements
				: grow_capacity(
					entry.capacity_elements,
					required_elements
				);
	...
	auto prefix = entry.backing->slice(
		0,
		0,
		required_elements
	);
...
Qwen2Workspace::position_values(
	std::size_t size
) {
	position_values_.resize(
		size
	);
