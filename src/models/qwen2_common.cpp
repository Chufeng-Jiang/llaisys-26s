...
void qwen2_copy_to_host(void *destination, const llaisys::tensor_t &source, std::size_t nbytes) {
    if (destination == nullptr) {
        throw std::invalid_argument("Qwen2 host destination must not be null.");
    }

    if (source == nullptr) { throw std::invalid_argument("Qwen2 source tensor must not be null."); }

    if (nbytes == 0) { return; }

    if (source->deviceType() == LLAISYS_DEVICE_CPU) {
        std::memcpy(destination, source->data(), nbytes);
        return;
    }
    ...
    llaisys::core::context().runtime().api()->memcpy_sync(
        destination, source->data(), nbytes, LLAISYS_MEMCPY_D2H);
}
