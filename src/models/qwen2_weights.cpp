...
using WeightArray =
	std::unique_ptr<llaisysTensor_t[]>;

WeightArray make_weight_array(
	std::size_t nlayer
) {
	return WeightArray(
...
void qwen2_allocate_weight_arrays(
...
void qwen2_release_weight_arrays(
	LlaisysQwen2Weights &weights
) noexcept {
...
void qwen2_validate_layer_weight_arrays(
...
Qwen2GlobalWeights qwen2_global_weights(
	const LlaisysQwen2Model &model
) {
...
Qwen2LayerWeights qwen2_layer_weights(
	const LlaisysQwen2Model &model,
	std::size_t layer
) {
	if (layer >= model.meta.nlayer) {
		throw std::out_of_range(
			"Qwen2 layer index is out of range."
		);
	}

	qwen2_validate_layer_weight_arrays(
		model
	);
