...
Qwen2PreparedInput qwen2_prepare_input(
	...
	auto position_ids =
		workspace.get(
			Qwen2WorkspaceSlot::PositionIds,
			{sequence_length},
			LLAISYS_DTYPE_I64
		);
	...
	llaisys::ops::embedding(
		hidden_states,
		input_ids,
		weights.input_embedding
	);

	return Qwen2PreparedInput{
		hidden_states,
		position_ids
	};
}
...
std::int64_t qwen2_predict_next_token(
	...
	if (hidden_states == nullptr) {
		throw std::invalid_argument(
			"Qwen2 output hidden state must not be null."
		);
	}
