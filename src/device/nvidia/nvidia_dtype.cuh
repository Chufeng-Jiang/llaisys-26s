...
template <typename T> struct CudaTypeTag {
    using type = T;
};
...
template <typename Fn> decltype(auto) dispatch_cuda_dtype(llaisysDataType_t type, Fn &&fn) {
    switch (type) {
    case LLAISYS_DTYPE_F32:
        return std::forward<Fn>(fn)(CudaTypeTag<float>{});

    case LLAISYS_DTYPE_F16:
        return std::forward<Fn>(fn)(CudaTypeTag<half>{});

    case LLAISYS_DTYPE_BF16:
        return std::forward<Fn>(fn)(CudaTypeTag<__nv_bfloat16>{});
