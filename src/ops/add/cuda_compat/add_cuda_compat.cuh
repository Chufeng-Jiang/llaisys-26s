...
template <typename T>
__device__ __forceinline__ T add_value(
	T a,
	T b
) {
	if constexpr (
...
inline bool can_use_vectorized_add(
	...
	constexpr std::size_t vector_size =
		VectorTraits<T>::ELEMENTS;

	constexpr std::size_t vector_alignment =
		VectorTraits<T>::ALIGNMENT;

	return
		numel >= vector_size
		&& are_aligned<vector_alignment>(
			c,
			a,
			b
		);
...
template <typename T>
inline std::size_t get_add_work_items(
	std::size_t numel,
...
__global__ void add_kernel_vectorized(
	...
	for (
		std::size_t vector_index = thread_index;
		...
		if constexpr (
			...
			c_vector.x =
				a_vector.x + b_vector.x;

			c_vector.y =
				a_vector.y + b_vector.y;

			c_vector.z =
				a_vector.z + b_vector.z;

			c_vector.w =
				a_vector.w + b_vector.w;
