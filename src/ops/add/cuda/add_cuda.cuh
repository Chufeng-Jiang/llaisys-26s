...
__device__ __forceinline__ T add_value(T a, T b) {
    if constexpr (std::is_same_v<T, float>) {
        ...
        static_assert(
            DEPENDENT_FALSE<T>,
            "Unsupported CUDA-compatible Add data type.");
    }
}
...
inline bool can_use_vectorized_add(
    ...
    return numel >= vector_size
        && are_aligned<vector_alignment>(c, a, b);
}
...
inline AddLaunchPlan get_launch_plan(
    ...
    const bool use_vectorized_kernel
        = enable_vectorized
        && can_use_vectorized_add<T>(c, a, b, numel);

    const std::size_t work_items
        = use_vectorized_kernel
        ? numel / VectorTraits<T>::ELEMENTS
        : numel;
...
__global__ void add_vectorized_kernel(
    ...
    for (std::size_t vector_index = thread_index;
        ...
        if constexpr (std::is_same_v<T, float>) {
            const float4 a_vector
                = *reinterpret_cast<const float4 *>(a + element_index);

            const float4 b_vector
                = *reinterpret_cast<const float4 *>(b + element_index);
            ...
            *reinterpret_cast<float4 *>(c + element_index)
                = c_vector;
        } else if constexpr (std::is_same_v<T, fp16_t>) {
            const fp16x2_t *const a_vector
                = reinterpret_cast<const fp16x2_t *>(a + element_index);

            const fp16x2_t *const b_vector
                = reinterpret_cast<const fp16x2_t *>(b + element_index);

            fp16x2_t *const c_vector
                = reinterpret_cast<fp16x2_t *>(c + element_index);

#pragma unroll
            for (int pair = 0; pair < 4; ++pair) {
                c_vector[pair]
                    = __hadd2(a_vector[pair], b_vector[pair]);
            }
        } else if constexpr (std::is_same_v<T, bf16_t>) {
            ...
            bf16x2_t *const c_vector
                = reinterpret_cast<bf16x2_t *>(c + element_index);
...
#pragma unroll
            ...
            static_assert(
                DEPENDENT_FALSE<T>,
                "Unsupported CUDA-compatible Add vector type.");
        }
    }

    const std::size_t tail_start
        = vector_count * vector_size;

    for (std::size_t index = tail_start + thread_index;
         index < numel;
         index += grid_stride) {
        c[index] = add_value<T>(a[index], b[index]);
    }
...
void launch_add(
    ...
    std::size_t numel,
    cudaStream_t stream) {
    const add_config::LaunchPolicy &policy
        = add_config::get_launch_policy();
    ...
    const AddLaunchPlan plan
        = get_launch_plan<T>(
            ...
            policy.block_size,
            policy.max_blocks,
            policy.enable_vectorized);

    if (config::debug_enabled()) {
        std::fprintf(
            stderr,
            "[Add][%s] implementation=shared numel=%zu "
            "kernel=%s vectorized_enabled=%s "
            "work_items=%zu block=%zu grid=%zu max_blocks=%zu\n",
            GPU_BACKEND_NAME,
            numel,
            plan.use_vectorized_kernel ? "vectorized" : "scalar",
            policy.enable_vectorized ? "true" : "false",
            plan.work_items,
            plan.block_size,
            plan.grid_size,
            policy.max_blocks);
    ...
    const dim3 block_dimension(
        static_cast<unsigned int>(plan.block_size));

    const dim3 grid_dimension(
        static_cast<unsigned int>(plan.grid_size));

    if (plan.use_vectorized_kernel) {
        add_vectorized_kernel<T>
            <<<grid_dimension, block_dimension, 0, stream>>>(
                c,
                a,
                b,
                numel);
