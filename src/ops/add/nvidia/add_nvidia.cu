...
__global__ void add_kernel_vectorized(T *c, const T *a, const T *b,
                                      std::size_t numel) {
    const std::size_t thread_idx = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
...
#pragma unroll
            ...
            const __nv_bfloat162 *a_vec = reinterpret_cast<const __nv_bfloat162 *>(a + index);

            const __nv_bfloat162 *b_vec = reinterpret_cast<const __nv_bfloat162 *>(b + index);
...
bool is_vector_aligned(const T *c, const T *a, const T *b) {
    ...
    return (c_address % alignment == 0 && a_address % alignment == 0 && b_address % alignment == 0);
}
...
void launch_add_kernel(T *c, const T *a, const T *b, std::size_t numel, cudaStream_t stream) {
    ...
    const bool use_vectorized_kernel = numel >= vector_size && is_vector_aligned(c, a, b);
