...
//          a, b, c, d]
template <typename T>
void embedding_impl(T *out, const std::int64_t *index, const T *weight,
  ...
  CHECK_ARGUMENT(numel <= std::numeric_limits<std::size_t>::max() / sizeof(T),
  ...
  for (std::size_t i = 0; i < index_count; ++i) {
    ...
    CHECK_ARGUMENT(static_cast<std::uint64_t>(raw_index) <
                       static_cast<std::uint64_t>(vocabulary_size),
                   "Embedding: index is out of vocabulary range.");
  }

  const bool use_openmp = index_count >= EMBEDDING_OPENMP_MIN_ROWS &&
                          total_bytes >= EMBEDDING_OPENMP_MIN_BYTES;
...
void embedding(std::byte *out, const std::byte *index, const std::byte *weight,
  ...
  switch (type) {
    ...
    case LLAISYS_DTYPE_BF16:
      return embedding_impl<llaisys::bf16_t>(
