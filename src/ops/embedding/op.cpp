...
void embedding(tensor_t out, tensor_t index, tensor_t weight) {
  ...
  CHECK_ARGUMENT(weight != nullptr,
                 "Embedding: weight tensor must not be null.");
  ...
  CHECK_ARGUMENT(index->ndim() == 1,
                 "Embedding: index tensor must be one-dimensional.");

  // weight shape:
  //
  //     [vocabulary_size, embedding_length]
  CHECK_ARGUMENT(weight->ndim() == 2,
                 "Embedding: weight tensor must be two-dimensional.");

  // out shape:
  //
  //     [index_count, embedding_length]
  CHECK_ARGUMENT(out->ndim() == 2,
                 "Embedding: output tensor must be two-dimensional.");
  ...
  CHECK_ARGUMENT(embedding_length > 0,
                 "Embedding: embedding length must be greater than zero.");

  CHECK_ARGUMENT(index_count == 0 || vocabulary_size > 0,
  ...
  CHECK_ARGUMENT(out->shape()[0] == index_count,
                 "Embedding: output row count must match index count.");

  // Output row length must equal weight row length.
  CHECK_ARGUMENT(out->shape()[1] == embedding_length,
  ...
  CHECK_ARGUMENT(index->dtype() == LLAISYS_DTYPE_I64,
                 "Embedding: index tensor must use int64 data type.");
  ...
  CHECK_ARGUMENT(out->deviceType() == index->deviceType() &&
                     out->deviceType() == weight->deviceType(),
                 "Embedding: all tensors must use the same device type.");

  CHECK_ARGUMENT(out->deviceId() == index->deviceId() &&
  ...
  CHECK_ARGUMENT(out->isContiguous(),
                 "Embedding: output tensor must be contiguous.");

  CHECK_ARGUMENT(index->isContiguous(),
                 "Embedding: index tensor must be contiguous.");

  CHECK_ARGUMENT(weight->isContiguous(),
                 "Embedding: weight tensor must be contiguous.");
  ...
  switch (out->deviceType()) {
    case LLAISYS_DEVICE_CPU:
      return cpu::embedding(out->data(), index->data(), weight->data(),
                            weight->dtype(), out->numel(), embedding_length,
                            vocabulary_size);
    ...
    case LLAISYS_DEVICE_NVIDIA: {
      ...
      return nvidia::embedding(out->data(), index->data(), weight->data(),
                               weight->dtype(), out->numel(), embedding_length,
                               vocabulary_size, runtime.stream());
    }
#endif
