...
void argmax(tensor_t max_idx, tensor_t max_val, tensor_t vals) {
  ...
  CHECK_ARGUMENT(max_idx != nullptr,
                 "Argmax: max_idx tensor must not be null.");

  CHECK_ARGUMENT(max_val != nullptr,
                 "Argmax: max_val tensor must not be null.");
  ...
  CHECK_ARGUMENT(max_idx->isContiguous(),
                 "Argmax: max_idx must be contiguous.");

  CHECK_ARGUMENT(max_val->isContiguous(),
                 "Argmax: max_val must be contiguous.");
  ...
  case LLAISYS_DEVICE_NVIDIA: {
    ...
    // The Runtime stores backend-specific resources through
    // the generic DeviceResource base class.
    auto *resource = static_cast<llaisys::device::nvidia::Resource *>(runtime.resource());

    CHECK_ARGUMENT(resource != nullptr,
                   "Argmax: NVIDIA Runtime resource must not be null.");

    // The workspace is allocated lazily on its first use
    // and then reused by subsequent Argmax calls.
    unsigned long long *packed_workspace = resource->argmaxPackedWorkspace();

    CHECK_ARGUMENT(packed_workspace != nullptr,
                   "Argmax: NVIDIA packed workspace must not be null.");

    nvidia::argmax(max_idx->data(), max_val->data(), vals->data(),
                   vals->dtype(), vals->numel(), packed_workspace,
                   runtime.stream());
