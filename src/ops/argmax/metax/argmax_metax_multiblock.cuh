...
struct ArgmaxReduceOp {
    __device__ ArgmaxResult operator()(const ArgmaxResult &left, const ArgmaxResult &right) const {
        ArgmaxResult result = left;

        if (is_valid_argmax_result(right)) {
            update_argmax_result(result, right.value, right.index);
        }
...
__global__ void argmax_multiblock_tree_stage2_kernel(
    ...
    const ArgmaxResult final_result = block_reduce_argmax_portable(thread_result, shared_results);
...
inline void launch_argmax_multiblock_tree(
    ...
    argmax_multiblock_tree_stage1_kernel<T>
        <<<grid_size, block_size, shared_memory_bytes, stream>>>(partial_results, vals, numel);
    ...
    argmax_multiblock_tree_stage2_kernel<T><<<1, block_size, shared_memory_bytes, stream>>>(
...
__global__ void argmax_multiblock_cub_stage1_kernel(
    ArgmaxResult *__restrict__ partial_results, const T *__restrict__ vals, std::size_t numel) {
    using BlockReduce = cub::BlockReduce<ArgmaxResult, BLOCK_SIZE>;
    ...
    const ArgmaxResult block_result
        = BlockReduce(reduction_storage).Reduce(thread_result, ArgmaxReduceOp{});
...
inline void launch_argmax_multiblock_cub_fixed(
    ...
    argmax_multiblock_cub_stage1_kernel<T, BLOCK_SIZE>
    ...
    argmax_multiblock_cub_stage2_kernel<T, BLOCK_SIZE><<<1, BLOCK_SIZE, 0, stream>>>(
        max_idx, max_val, partial_results, static_cast<std::size_t>(grid_size));
...
inline void launch_argmax_multiblock_cub(
    ...
    switch (block_size) {
    case 64:
        return launch_argmax_multiblock_cub_fixed<T, 64>(
            max_idx, max_val, vals, numel, partial_results, grid_size, stream);

    case 128:
        return launch_argmax_multiblock_cub_fixed<T, 128>(
            max_idx, max_val, vals, numel, partial_results, grid_size, stream);

    case 256:
        return launch_argmax_multiblock_cub_fixed<T, 256>(
            max_idx, max_val, vals, numel, partial_results, grid_size, stream);
