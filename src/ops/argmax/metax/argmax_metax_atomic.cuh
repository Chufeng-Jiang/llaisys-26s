...
static_assert(
    sizeof(unsigned long long) == 8, "MetaX atomic Argmax requires 64-bit unsigned long long.");
...
atomic_merge_argmax_result(unsigned long long *packed_workspace, const ArgmaxResult &candidate) {
...
__global__ void initialize_argmax_atomic_workspace_kernel(unsigned long long *packed_workspace) {
...
__global__ void argmax_multiblock_shuffle_atomic_kernel(
    ...
    if (threadIdx.x == 0) { atomic_merge_argmax_result(packed_workspace, block_result); }
}
...
template <typename T>
__global__ void finalize_argmax_atomic_result_kernel(
...
template <typename T, unsigned int BLOCK_SIZE>
inline void launch_argmax_multiblock_shuffle_atomic_fixed(
    std::int64_t *max_idx,
    T *max_val,
    const T *vals,
    std::size_t numel,
    ...
    mcStream_t stream) {
    initialize_argmax_atomic_workspace_kernel<<<1, 1, 0, stream>>>(packed_workspace);

    MC_CHECK(mcGetLastError());

    argmax_multiblock_shuffle_atomic_kernel<T, BLOCK_SIZE>
        <<<grid_size, BLOCK_SIZE, 0, stream>>>(packed_workspace, vals, numel);
...
template <typename T>
inline void launch_argmax_multiblock_shuffle_atomic(
    std::int64_t *max_idx,
    T *max_val,
    const T *vals,
    std::size_t numel,
    ...
    switch (block_size) {
    case 64:
        return launch_argmax_multiblock_shuffle_atomic_fixed<T, 64>(
            max_idx, max_val, vals, numel, packed_workspace, grid_size, stream);

    case 128:
        return launch_argmax_multiblock_shuffle_atomic_fixed<T, 128>(
            max_idx, max_val, vals, numel, packed_workspace, grid_size, stream);

    case 256:
        return launch_argmax_multiblock_shuffle_atomic_fixed<T, 256>(
            max_idx, max_val, vals, numel, packed_workspace, grid_size, stream);
