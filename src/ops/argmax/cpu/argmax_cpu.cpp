...
/* Define the accumulator type used for internal computation. */
template <typename T>
...
/* Store the maximum value, its index, and validity state. */
template <typename T> struct MaxResult {
	T value{};
	std::int64_t index{0};
	bool valid{false};
};

/* Convert a value to the accumulator type for internal computation. */
template <typename T> accumulator_t<T> to_accumulator(T value) {
...
/* Convert an accumulator value back to the original data type. */
template <typename T> T from_accumulator(accumulator_t<T> value) {
	if constexpr (std::is_same_v<T, llaisys::fp16_t> || std::is_same_v<T, llaisys::bf16_t>) {
...
/* Compare two values and select the better maximum candidate. */
template <typename T>
bool is_better(
	T candidate_value, std::int64_t candidate_index, T current_value, std::int64_t current_index) {
...
/* Merge two partial maximum results into one result. */
MaxResult<float> merge_max_results(const MaxResult<float> &left, const MaxResult<float> &right) {
	...
	if (is_better(right.value, right.index, left.value, left.index)) { return right; }
	return left;
}


/* Define the OpenMP argmax reduction. */
#pragma omp declare reduction(                                                                     \
		llaisys_argmax_reduction : MaxResult<float> : omp_out                                      \
			= merge_max_results(omp_out, omp_in)) initializer(omp_priv = MaxResult<float>())

/* Compute the maximum value and its index using optional OpenMP parallel reduction. */
template <typename T>
void argmax_impl(std::int64_t *max_idx, T *max_val, const T *vals, std::size_t numel) {
	...
	static_assert(std::is_same_v<AccumulatorType, float>, "Argmax: unsupported accumulator type.");
