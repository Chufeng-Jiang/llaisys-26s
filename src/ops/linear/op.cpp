...
void linear(tensor_t out, tensor_t in, tensor_t weight, tensor_t bias) {
    ...
    CHECK_ARGUMENT(out->ndim() == 2,
                   "Linear: output tensor must be two-dimensional.");

    CHECK_ARGUMENT(in->ndim() == 2,
                   "Linear: input tensor must be two-dimensional.");

    CHECK_ARGUMENT(weight->ndim() == 2,
                   "Linear: weight tensor must be two-dimensional.");
    ...
    CHECK_ARGUMENT(weight->shape()[1] == input_features,
    ...
    CHECK_ARGUMENT(out->shape()[1] == output_features,
                   "Linear: output feature count must match weight row count.");

    if (bias != nullptr) {
        CHECK_ARGUMENT(bias->ndim() == 1,
                       "Linear: bias tensor must be one-dimensional.");

        CHECK_ARGUMENT(bias->shape()[0] == output_features,
    ...
    if (bias != nullptr) {
        CHECK_ARGUMENT(bias->dtype() == out->dtype(),
                       "Linear: bias must use the same data type as output.");
    }
    ...
    CHECK_ARGUMENT(out->deviceId() == in->deviceId() && out->deviceId() == weight->deviceId(),
    ...
    CHECK_ARGUMENT(out->isContiguous(),
                   "Linear: output tensor must be contiguous.");

    CHECK_ARGUMENT(in->isContiguous(),
                   "Linear: input tensor must be contiguous.");

    CHECK_ARGUMENT(weight->isContiguous(),
                   "Linear: weight tensor must be contiguous.");

    if (bias != nullptr) {
        CHECK_ARGUMENT(bias->isContiguous(),
                       "Linear: bias tensor must be contiguous.");
    }
    ...
    case LLAISYS_DEVICE_CPU:
        return cpu::linear(out->data(), in->data(), weight->data(),
                           ...
                           row_count, output_features, input_features);
    ...
    case LLAISYS_DEVICE_NVIDIA: {
        ...
        return nvidia::linear(out->data(), in->data(), weight->data(),
                              ...
                              out->dtype(), row_count, output_features,
                              input_features, runtime.stream());
    }
#endif
