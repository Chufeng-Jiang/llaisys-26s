...
inline float dot_product(const float *a, const float *b, std::size_t count) {
    ...
    for (; k + 63 < count; k += 64) {
        sum0 = _mm512_fmadd_ps(_mm512_loadu_ps(a + k), _mm512_loadu_ps(b + k), sum0);
        sum1 = _mm512_fmadd_ps(_mm512_loadu_ps(a + k + 16), _mm512_loadu_ps(b + k + 16), sum1);
        sum2 = _mm512_fmadd_ps(_mm512_loadu_ps(a + k + 32), _mm512_loadu_ps(b + k + 32), sum2);
        sum3 = _mm512_fmadd_ps(_mm512_loadu_ps(a + k + 48), _mm512_loadu_ps(b + k + 48), sum3);
    }
...
#else
        sum0 = _mm256_add_ps(sum0, _mm256_mul_ps(_mm256_loadu_ps(a + k), _mm256_loadu_ps(b + k)));
        sum1 = _mm256_add_ps(
            sum1, _mm256_mul_ps(_mm256_loadu_ps(a + k + 8), _mm256_loadu_ps(b + k + 8)));
        sum2 = _mm256_add_ps(
            sum2, _mm256_mul_ps(_mm256_loadu_ps(a + k + 16), _mm256_loadu_ps(b + k + 16)));
        sum3 = _mm256_add_ps(
            sum3, _mm256_mul_ps(_mm256_loadu_ps(a + k + 24), _mm256_loadu_ps(b + k + 24)));
#endif
    }
...
#pragma omp simd reduction(+ : sum)
    for (std::size_t tail = k; tail < count; ++tail) { sum += a[tail] * b[tail]; }
...
void vecmul(const float *a, const float *B, float *c, std::size_t N, std::size_t K) {
    ...
    const bool use_openmp = N >= 8 && K >= 256;

#pragma omp parallel for if (use_openmp) schedule(static)
    for (std::size_t row = 0; row < N; ++row) {
        const float *const weight_row = B + row * K;

        c[row] += dot_product(a, weight_row, K);
    }
}
