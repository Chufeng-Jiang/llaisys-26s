...
bool multiplication_fits(std::size_t left, std::size_t right) {
...
void prepare_frequency_denominator(RopeWorkspace &workspace, float theta, std::size_t dimension) {
    if (workspace.frequency_cache_valid && workspace.cached_dimension == dimension
...
template <typename T> inline float load_as_float(T value) {
...
template <typename T> inline T store_from_float(float value) {
    if constexpr (std::is_same_v<T, float>) {
        return value;
    } else {
...
void apply_rope_vector(
    T *out, const T *in, const float *cosine, const float *sine, std::size_t half_dimension) {
...
#pragma omp simd
    for (std::size_t index = 0; index < half_dimension; ++index) {
        ...
        out[index + half_dimension]
            = store_from_float<T>(second * cosine_value + first * sine_value);
    }
}
...
void rope_impl(
    ...
    static_assert(
        ...
        "RoPE: unsupported CPU element type.");
    ...
    CHECK_ARGUMENT(
        multiplication_fits(sequence_length, head_count),
        "RoPE: sequence length and head count overflow size_t.");

    const std::size_t vector_count = sequence_length * head_count;

    CHECK_ARGUMENT(
        multiplication_fits(vector_count, dimension),
    ...
    CHECK_ARGUMENT(
        sequence_length == 0 || pos_ids != nullptr, "RoPE: position ID pointer must not be null.");
    ...
    CHECK_ARGUMENT(
        multiplication_fits(sequence_length, dimension),
        "RoPE: trigonometric cache size overflows size_t.");
