...
void launch_nvidia_swiglu(
    T *out, const T *gate, const T *up, std::size_t numel, cudaStream_t stream) {
    if (numel == 0) { return; }
    ...
    const bool use_packed_kernel = cuda_compat::can_use_packed_swiglu<T>(out, gate, up, numel);

    const std::size_t work_items = cuda_compat::get_swiglu_work_items<T>(numel, use_packed_kernel);
    ...
    cuda_compat::launch_swiglu_kernel<T>(
        out, gate, up, numel, block_size, grid_size, use_packed_kernel, stream);
