...
void swiglu(
	...
	CHECK_ARGUMENT(
		out != nullptr,
		"SwiGLU: output tensor must not be null."
	);

	CHECK_ARGUMENT(
		gate != nullptr,
		"SwiGLU: gate tensor must not be null."
	);

	CHECK_ARGUMENT(
		up != nullptr,
		"SwiGLU: up tensor must not be null."
	);
	...
	CHECK_ARGUMENT(
		out->ndim() == 2,
		"SwiGLU: output tensor must be two-dimensional."
	);

	CHECK_ARGUMENT(
		gate->ndim() == 2,
		"SwiGLU: gate tensor must be two-dimensional."
	);

	CHECK_ARGUMENT(
		up->ndim() == 2,
		"SwiGLU: up tensor must be two-dimensional."
	);
	...
	CHECK_ARGUMENT(
		...
		"SwiGLU: output and gate tensors must have the same shape."
	);

	CHECK_ARGUMENT(
		out->shape() == up->shape(),
		"SwiGLU: output and up tensors must have the same shape."
	);
	...
	CHECK_ARGUMENT(
		out->dtype() == up->dtype(),
		"SwiGLU: output and up tensors must use the same data type."
	);
	...
	CHECK_ARGUMENT(
		out->isContiguous(),
		"SwiGLU: output tensor must be contiguous."
	);

	CHECK_ARGUMENT(
		gate->isContiguous(),
		"SwiGLU: gate tensor must be contiguous."
	);

	CHECK_ARGUMENT(
		up->isContiguous(),
		"SwiGLU: up tensor must be contiguous."
	);
	...
	default:
		CHECK_ARGUMENT(
			false,
			"SwiGLU: unsupported device type."
		);
