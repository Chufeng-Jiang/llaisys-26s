...
template <typename T>
__global__ void swiglu_scalar_kernel(T *out, const T *gate, const T *up, std::size_t numel) {
    ...
    for (std::size_t index = thread_index; index < numel; index += thread_stride) {
...
template <typename T>
__global__ void swiglu_packed_kernel(T *out, const T *gate, const T *up, std::size_t numel) {
    constexpr std::size_t elements_per_pack = PACKED_128_ELEMENTS<T>;

    static_assert(elements_per_pack > 0, "Packed128 must contain at least one element.");
    ...
    const Packed128 *gate_packs = reinterpret_cast<const Packed128 *>(gate);

    const Packed128 *up_packs = reinterpret_cast<const Packed128 *>(up);

    Packed128 *out_packs = reinterpret_cast<Packed128 *>(out);
    ...
    for (std::size_t pack_index = thread_index; pack_index < pack_count;
        ...
        T *out_values = reinterpret_cast<T *>(&out_pack);
...
#pragma unroll
    ...
    for (std::size_t index = tail_start + thread_index; index < numel; index += thread_stride) {
        const T gate_value = gate[index];
...
inline void launch_swiglu_kernel(
    ...
    if (use_packed_kernel) {
        swiglu_packed_kernel<T>
            <<<static_cast<unsigned int>(grid_size), block_size, 0, stream>>>(out, gate, up, numel);
    } else {
