...
float to_float(T value) {
	if constexpr (std::is_same_v<T, float>) {
		...
		static_assert(
			IS_LOW_PRECISION<T>,
			"RMSNorm: unsupported conversion source type."
		);
...
T from_float(float value) {
	if constexpr (std::is_same_v<T, float>) {
		...
		static_assert(
			IS_LOW_PRECISION<T>,
			"RMSNorm: unsupported conversion destination type."
		);
...
template <typename T>
float calculate_square_sum(
	const T *input,
	std::size_t count
) {
	if constexpr (std::is_same_v<T, float>) {
		return llaisys::ops::cpu::sdot(
			input,
			input,
			count
		);
...
#pragma omp simd
		for (std::size_t i = 0; i < count; ++i) {
			out[i] = in[i] * weight[i] * inverse_rms;
		}
		...
		// Decode-like single-row calls avoid allocating a temporary FP32 weight.
		for (std::size_t i = 0; i < count; ++i) {
			const float value =
				to_float(in[i])
				* to_float(weight[i])
...
void rms_norm_impl(
	...
	CHECK_ARGUMENT(
		column_count > 0,
		"RMSNorm: column count must be greater than zero."
	);
	...
	CHECK_ARGUMENT(
		eps >= 0.0F,
		"RMSNorm: epsilon must not be negative."
	);
...
#pragma omp simd
			for (std::size_t column = 0;
				column < column_count;
				++column) {
				converted_weight_storage[column] =
					to_float(weight[column]);
			}

			converted_weight =
				converted_weight_storage.data();
		}
	}

	const bool use_openmp =
		element_count >= OPENMP_THRESHOLD
		&& row_count > 1;

#pragma omp parallel for if(use_openmp) schedule(static)
	for (std::size_t row = 0; row < row_count; ++row) {
		T *out_row =
			out + row * column_count;
