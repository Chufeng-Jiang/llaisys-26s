...
float sdot_avx512(
	...
	for (; i + 64 <= count; i += 64) {
		...
		sum2 = _mm512_fmadd_ps(
			_mm512_loadu_ps(x + i + 32),
			_mm512_loadu_ps(y + i + 32),
			sum2
		);

		sum3 = _mm512_fmadd_ps(
			_mm512_loadu_ps(x + i + 48),
...
float sdot(
	...
	static const SdotFunction implementation =
		select_sdot_implementation();
