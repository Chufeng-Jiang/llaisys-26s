...
void rms_norm(tensor_t out, tensor_t in, tensor_t weight, float eps) {
  ...
  CHECK_ARGUMENT(in->ndim() == 2,
                 "RMSNorm: input tensor must be two-dimensional.");

  CHECK_ARGUMENT(weight->ndim() == 1,
                 "RMSNorm: weight tensor must be one-dimensional.");
  ...
  CHECK_ARGUMENT(column_count > 0,
                 "RMSNorm: input row length must be greater than zero.");

  // Output shape must match input shape exactly.
  CHECK_ARGUMENT(out->shape()[0] == row_count,
                 "RMSNorm: output row count must match input row count.");

  CHECK_ARGUMENT(out->shape()[1] == column_count,
                 "RMSNorm: output column count must match input column count.");

  // One scaling weight is required for every column.
  CHECK_ARGUMENT(weight->shape()[0] == column_count,
                 "RMSNorm: weight length must match input row length.");
  ...
  CHECK_ARGUMENT(out->dtype() == in->dtype(),
                 "RMSNorm: output and input must use the same data type.");

  CHECK_ARGUMENT(weight->dtype() == in->dtype(),
                 "RMSNorm: weight and input must use the same data type.");

  // ============================================================
  // Device checks
  // ============================================================

  CHECK_ARGUMENT(out->deviceType() == in->deviceType(),
                 "RMSNorm: output and input must use the same device type.");

  CHECK_ARGUMENT(weight->deviceType() == in->deviceType(),
  ...
  CHECK_ARGUMENT(out->isContiguous(),
                 "RMSNorm: output tensor must be contiguous.");

  CHECK_ARGUMENT(in->isContiguous(),
                 "RMSNorm: input tensor must be contiguous.");

  CHECK_ARGUMENT(weight->isContiguous(),
                 "RMSNorm: weight tensor must be contiguous.");
  ...
  switch (out->deviceType()) {
    case LLAISYS_DEVICE_CPU:
      return cpu::rms_norm(out->data(), in->data(), weight->data(), eps,
                           out->dtype(), row_count, column_count);
    ...
    case LLAISYS_DEVICE_NVIDIA: {
      ...
      return nvidia::rms_norm(out->data(), in->data(), weight->data(), eps,
                              out->dtype(), row_count, column_count,
                              runtime.stream());
    }
#endif
