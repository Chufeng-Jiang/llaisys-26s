...
public:
    OffsetCursor(
        ...
        : _shape(shape), _strides(strides), _coordinates(dimension_count, 0),
          _dimension_count(dimension_count) {
        for (std::size_t dimension = dimension_count; dimension-- > 0;) {
            const std::size_t extent = _shape[dimension];
            ...
            _offset += static_cast<std::ptrdiff_t>(coordinate) * _strides[dimension];
        }
    }
    ...
    void advance() {
        for (std::size_t dimension = _dimension_count; dimension-- > 0;) {
            ++_coordinates[dimension];

            _offset += _strides[dimension];

            if (_coordinates[dimension] < _shape[dimension]) { return; }

            _coordinates[dimension] = 0;

            _offset -= static_cast<std::ptrdiff_t>(_shape[dimension]) * _strides[dimension];
        }
    }
...
std::pair<std::size_t, std::size_t> worker_range(std::size_t work_items) {
    const std::size_t thread_index = static_cast<std::size_t>(omp_get_thread_num());

    const std::size_t thread_count = static_cast<std::size_t>(omp_get_num_threads());

    const std::size_t begin = work_items * thread_index / thread_count;

    const std::size_t end = work_items * (thread_index + 1) / thread_count;
...
template <typename T> void copy_fully_contiguous(T *out, const T *in, std::size_t numel) {
    std::memmove(out, in, numel * sizeof(T));
}
...
void copy_contiguous_tail(
    ...
    bool use_openmp) {
    const std::size_t block_count = numel / tail.element_count;

    const std::size_t block_bytes = tail.element_count * sizeof(T);

#pragma omp parallel if (use_openmp)
    {
        const auto [begin, end] = worker_range(block_count);

        if (begin < end) {
            OffsetCursor out_cursor(shape, out_strides, tail.start_dimension, begin);

            OffsetCursor in_cursor(shape, in_strides, tail.start_dimension, begin);

            for (std::size_t block = begin; block < end; ++block) {
                std::memmove(out + out_cursor.offset(), in + in_cursor.offset(), block_bytes);
...
#pragma omp parallel if (use_openmp)
    {
        const auto [begin, end] = worker_range(numel);

        if (begin < end) {
            OffsetCursor out_cursor(out_shape, out_strides, out_shape.size(), begin);

            OffsetCursor in_cursor(in_shape, in_strides, in_shape.size(), begin);

            for (std::size_t index = begin; index < end; ++index) {
                out[out_cursor.offset()] = in[in_cursor.offset()];
...
void rearrange_typed(
    ...
    if (is_contiguous_layout(out_shape, out_strides)
        && is_contiguous_layout(in_shape, in_strides)) {
        return copy_fully_contiguous(out, in, numel);
    }

    const bool output_is_non_overlapping = is_non_overlapping_layout(out_shape, out_strides);
    ...
    const bool can_parallelize = output_is_non_overlapping && out != in;

    const bool use_openmp = can_parallelize && numel >= OPENMP_THRESHOLD;
    ...
    if (out_shape == in_shape) {
        ...
        if (tail.element_count > 1 && tail.element_count * sizeof(T) >= CONTIGUOUS_TAIL_MIN_BYTES) {
            return copy_contiguous_tail(
                out, in, numel, out_shape, out_strides, in_strides, tail, use_openmp);
        }
    }
    ...
    copy_incrementally(out, in, numel, out_shape, out_strides, in_shape, in_strides, use_openmp);
}
