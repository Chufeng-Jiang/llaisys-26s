...
AddressRange layout_address_range(
	...
	return AddressRange{
		base_address
			+ minimum_offset * element_bytes,
		base_address
			+ maximum_offset * element_bytes
			+ element_bytes,
	};
}

bool address_ranges_overlap(
...
bool can_use_packed_tail(
	...
	if (
		tail.element_count % elements_per_pack != 0
	...
	for (
		std::size_t dimension = 0;
		dimension < tail.start_dimension;
		++dimension
	) {
		if (shape[dimension] <= 1) {
			continue;
...
__device__ __forceinline__ std::ptrdiff_t logical_offset(
...
__global__ void rearrange_generic_kernel(
	...
	for (
		std::size_t logical_index = thread_index;
		logical_index < numel;
...
template <typename T>
__global__ void gather_to_contiguous_kernel(
	T *temporary,
	const T *in,
	std::size_t numel,
	DeviceLayout in_layout
...
__global__ void rearrange_contiguous_tail_packed_kernel(
	...
	for (
		...
		Packed128 *packed_out =
			reinterpret_cast<Packed128 *>(
				out + out_offset
			);

		const Packed128 *packed_in =
			reinterpret_cast<const Packed128 *>(
				in + in_offset
			);
...
void launch_contiguous_tail_copy(
	...
	const std::size_t outer_block_count =
	...
	if (
		...
		const std::size_t packs_per_outer_block =
			tail.element_count
			/ elements_per_pack;
...
void rearrange_typed(
	...
	const bool in_is_contiguous =
		is_contiguous_layout(
			in_shape,
			in_strides
		);

	const bool storage_overlaps =
		address_ranges_overlap(
			layout_address_range(
				out,
				out_shape,
				out_strides
			),
...
void rearrange(
	...
	static_assert(
		...
		"Rearrange: Float32 must occupy four bytes."
	);

	static_assert(
		sizeof(llaisys::fp16_t) == sizeof(std::uint16_t),
		"Rearrange: FP16 must occupy two bytes."
	);

	static_assert(
		sizeof(llaisys::bf16_t) == sizeof(std::uint16_t),
