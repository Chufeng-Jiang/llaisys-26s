...
#if defined(ENABLE_CUDNN_API) && __has_include(<cudnn.h>) \
...
// ============================================================
// NVIDIA SelfAttention implementation selection
// ============================================================
//
// 0:
//     Production NVIDIA implementation:
//
//         cuDNN SDPA
//             ↓ fallback
//         NVIDIA fused CUDA kernel
//
// 1:
//     Shared CUDA-compatible portability baseline.
//
// Production builds should keep this disabled.
// ============================================================

#ifndef LLAISYS_NVIDIA_SELF_ATTENTION_USE_CUDA_COMPAT
#define LLAISYS_NVIDIA_SELF_ATTENTION_USE_CUDA_COMPAT 0
#endif
...
inline constexpr std::size_t WARP_COUNT = CUDA_BLOCK_SIZE / CUDA_WARP_SIZE;

static_assert(
    CUDA_BLOCK_SIZE % CUDA_WARP_SIZE == 0, "SelfAttention: CUDA block size must be warp aligned.");

static_assert(
    WARP_COUNT <= llaisys::device::nvidia::CUDA_MAX_WARPS_PER_BLOCK,
    "SelfAttention: warp count exceeds the common CUDA limit.");

__device__ __forceinline__ float warp_reduce_sum(float value) {
#pragma unroll
    for (int offset = static_cast<int>(CUDA_WARP_SIZE / 2); offset > 0; offset /= 2) {
...
__device__ __forceinline__ float block_reduce_sum(float value, float *shared_reduction) {
    const unsigned int lane = threadIdx.x % CUDA_WARP_SIZE;
    const unsigned int warp = threadIdx.x / CUDA_WARP_SIZE;

    value = warp_reduce_sum(value);

    if (lane == 0) { shared_reduction[warp] = value; }
    ...
    if (warp == 0) {
        block_value = lane < WARP_COUNT ? shared_reduction[lane] : 0.0F;

        block_value = warp_reduce_sum(block_value);

        if (lane == 0) { shared_reduction[0] = block_value; }
    }
...
__device__ __forceinline__ float block_reduce_max(float value, float *shared_reduction) {
    const unsigned int lane = threadIdx.x % CUDA_WARP_SIZE;
    const unsigned int warp = threadIdx.x / CUDA_WARP_SIZE;
    ...
    if (warp == 0) {
        block_value = lane < WARP_COUNT ? shared_reduction[lane] : -FLT_MAX;
...
// NVIDIA fused fallback used for:
//
// - FP32;
...
__global__ void self_attention_fallback_kernel(
    ...
    for (std::size_t task = blockIdx.x; task < task_count; task += gridDim.x) {
        ...
        for (std::size_t index = threadIdx.x; index < d; index += blockDim.x) {
            shared_q[index] = to_float<T>(query[index]);
        }

        for (std::size_t index = threadIdx.x; index < dv; index += blockDim.x) {
        ...
        for (std::size_t key_start = 0; key_start < causal_length; key_start += TILE_KV) {
            const std::size_t remaining_keys = causal_length - key_start;

            const std::size_t tile_length = remaining_keys < TILE_KV ? remaining_keys : TILE_KV;

            // Each warp computes one score at a time.
            for (std::size_t local_key = warp; local_key < tile_length; local_key += WARP_COUNT) {
                const std::size_t key_index = key_start + local_key;

                const T *key = k + (key_index * nkvhead + kv_head) * d;

                float dot = 0.0F;

                for (std::size_t index = lane; index < d; index += CUDA_WARP_SIZE) {
                    dot += shared_q[index] * to_float<T>(key[index]);
                }

                dot = warp_reduce_sum(dot);

                if (lane == 0) { shared_scores[local_key] = dot * scale; }
            }
            ...
            for (std::size_t index = threadIdx.x; index < tile_length; index += blockDim.x) {
                local_max = fmaxf(local_max, shared_scores[index]);
            }

            const float tile_max = block_reduce_max(local_max, shared_reduction);

            float local_sum = 0.0F;

            for (std::size_t index = threadIdx.x; index < tile_length; index += blockDim.x) {
                const float probability = expf(shared_scores[index] - tile_max);

                shared_scores[index] = probability;
                local_sum += probability;
            }

            const float tile_sum = block_reduce_sum(local_sum, shared_reduction);

            if (threadIdx.x == 0) {
                const float running_max = shared_state[0];
                const float new_max = fmaxf(running_max, tile_max);

                shared_state[2] = running_max == -FLT_MAX ? 0.0F : expf(running_max - new_max);
            ...
            for (std::size_t value_index = threadIdx.x; value_index < dv;
                 value_index += blockDim.x) {
                float weighted_value = 0.0F;

                for (std::size_t local_key = 0; local_key < tile_length; ++local_key) {
                    const std::size_t key_index = key_start + local_key;

                    const T *value = v + (key_index * nkvhead + kv_head) * dv;

                    weighted_value += shared_scores[local_key] * to_float<T>(value[value_index]);
                }

                shared_output[value_index]
                    = shared_output[value_index] * old_scale + weighted_value * tile_scale;
            }

            if (threadIdx.x == 0) {
                shared_state[1] = shared_state[1] * old_scale + tile_sum * tile_scale;
            }
        ...
        for (std::size_t index = threadIdx.x; index < dv; index += blockDim.x) {
            output[index] = from_float<T>(shared_output[index] / shared_state[1]);
        }
...
void launch_fallback(
    ...
    const std::size_t task_count
        = checked_product(seqlen, nhead, "SelfAttention: task count overflows size_t.");
    ...
    CHECK_ARGUMENT(
        dv <= std::numeric_limits<std::size_t>::max() - partial_shared_elements - WARP_COUNT - 4,
        "SelfAttention: shared-memory element count overflows size_t.");

    const std::size_t shared_elements = partial_shared_elements + dv + WARP_COUNT + 4;

    CHECK_ARGUMENT(
        shared_elements <= std::numeric_limits<std::size_t>::max() / sizeof(float),
        "SelfAttention: shared-memory byte count overflows size_t.");

    const std::size_t shared_bytes = shared_elements * sizeof(float);

    int device = -1;
    CUDA_CHECK(cudaGetDevice(&device));

    const cudaDeviceProp &properties = get_device_properties(device);
    ...
    CHECK_ARGUMENT(
        ...
        "SelfAttention: d and dv require more shared memory than this GPU "
        "supports.");
...
// ============================================================
//
// The portable kernel itself contains no:
//
//     warp shuffle
...
//     block size
//     grid size
//     device shared-memory capability query
//     dynamic shared-memory opt-in
//     CUDA error handling
// ============================================================

template <typename T>
void launch_cuda_compat_baseline(
    T *attn_val,
    const T *q,
    const T *k,
    const T *v,
    ...
    CHECK_ARGUMENT(
        static_cast<std::size_t>(CUDA_BLOCK_SIZE)
            <= std::numeric_limits<std::size_t>::max() - q_output_elements - 4,
        "SelfAttention: shared-memory element count overflows size_t.");

    const std::size_t shared_elements
        = q_output_elements + static_cast<std::size_t>(CUDA_BLOCK_SIZE) + 4;
...
void check_cudnn(cudnnStatus_t status, const char *operation) {
    ...
    throw std::runtime_error(
        std::string("SelfAttention: ") + operation + " failed: " + cudnnGetErrorString(status));
}
...
struct CudnnGraphKey {
    ...
    bool operator==(const CudnnGraphKey &other) const noexcept {
        return device == other.device && type == other.type && seqlen == other.seqlen
            && nhead == other.nhead && dv == other.dv && total_len == other.total_len
            && nkvhead == other.nkvhead && d == other.d && scale_bits == other.scale_bits;
    }
};
...
public:
    explicit CudnnGraphEntry(const CudnnGraphKey &key) : _device_id(key.device) {
        ...
        try {
            ...
            const fe::DataType_t io_type
                = key.type == LLAISYS_DTYPE_F16 ? fe::DataType_t::HALF : fe::DataType_t::BFLOAT16;

            _graph = std::make_shared<fe::graph::Graph>();
            _graph->set_io_data_type(io_type)
                .set_intermediate_data_type(fe::DataType_t::FLOAT)
                .set_compute_data_type(fe::DataType_t::FLOAT);

            const std::int64_t sequence_length = static_cast<std::int64_t>(key.seqlen);
            const std::int64_t total_length = static_cast<std::int64_t>(key.total_len);
            const std::int64_t query_heads = static_cast<std::int64_t>(key.nhead);
            const std::int64_t kv_heads = static_cast<std::int64_t>(key.nkvhead);
            const std::int64_t query_dimension = static_cast<std::int64_t>(key.d);
            const std::int64_t value_dimension = static_cast<std::int64_t>(key.dv);
            ...
            const auto build_error = _graph->build(_handle, {fe::HeurMode_t::A});

            if (!build_error.is_good()) {
                throw std::runtime_error("SelfAttention: cuDNN SDPA graph build failed.");
            }

            std::int64_t workspace_size = 0;
            const auto workspace_error = _graph->get_workspace_size(workspace_size);

            if (!workspace_error.is_good() || workspace_size < 0) {
                throw std::runtime_error("SelfAttention: cuDNN SDPA workspace query failed.");
            }

            _workspace_size = static_cast<std::size_t>(workspace_size);

            if (_workspace_size > 0) { CUDA_CHECK(cudaMalloc(&_workspace, _workspace_size)); }
        } catch (...) {
    ...
    ~CudnnGraphEntry() noexcept {
        run_on_cuda_device_noexcept(
            _device_id, [this]() noexcept { release_resources_noexcept(); });
    }

    CudnnGraphEntry(const CudnnGraphEntry &) = delete;
    CudnnGraphEntry &operator=(const CudnnGraphEntry &) = delete;

    void execute(void *output, const void *q, const void *k, const void *v, cudaStream_t stream) {
        check_cudnn(cudnnSetStream(_handle, stream), "cudnnSetStream");
        ...
        const auto execute_error = _graph->execute(_handle, variant_pack, _workspace);

        if (!execute_error.is_good()) {
            throw std::runtime_error("SelfAttention: cuDNN SDPA execution failed.");
        }
    }
...
public:
    CudnnGraphEntry *find(const CudnnGraphKey &key) {
        ...
        if (iterator == _index.end()) { return nullptr; }
        ...
        _entries.splice(_entries.begin(), _entries, iterator->second);

        return iterator->second->entry.get();
    }

    void insert(const CudnnGraphKey &key, std::unique_ptr<CudnnGraphEntry> entry) {
        auto existing = _index.find(key);

        if (existing != _index.end()) {
            existing->second->entry = std::move(entry);
            _entries.splice(_entries.begin(), _entries, existing->second);
            return;
        }
...
private:
    ...
    EntryList _entries;
    std::unordered_map<CudnnGraphKey, EntryList::iterator, CudnnGraphKeyHash> _index;
};
...
public:
    bool contains(const CudnnGraphKey &key) {
        ...
        if (iterator == _index.end()) { return false; }
    ...
    void insert(const CudnnGraphKey &key) {
        ...
        try {
            _index.emplace(_entries.front(), _entries.begin());
        } catch (...) {
            _entries.pop_front();
            throw;
        }

        while (_entries.size() > CUDNN_REJECTED_CACHE_CAPACITY_PER_DEVICE) {
...
bool cudnn_supports(
    ...
    std::size_t d,
    std::size_t dv) {
    if (type != LLAISYS_DTYPE_F16 && type != LLAISYS_DTYPE_BF16) { return false; }

    if (seqlen == 0 || total_len == 0 || d % 8 != 0 || dv % 8 != 0) { return false; }

#if defined(CUDART_VERSION)
    if (CUDART_VERSION < 12000) { return false; }
#endif
    ...
    try {
        ...
        // fall back to the NVIDIA fused CUDA implementation instead of
        // making cuDNN support mandatory.
        return false;
    }

    if (properties->major < 8) { return false; }

    const std::size_t maximum_dimension = properties->major >= 9 ? 256 : 128;

    return d <= maximum_dimension && dv <= maximum_dimension;
}
...
bool try_cudnn(
    ...
    std::size_t d,
    cudaStream_t stream) {
    if (!cudnn_supports(type, seqlen, total_len, d, dv)) { return false; }
    ...
    if (device_cache.rejected.contains(key)) { return false; }
    ...
    if (entry == nullptr) {
        ...
        entry = new_entry.get();
        device_cache.graphs.insert(key, std::move(new_entry));
    }
    ...
    entry->execute(attn_val, q, k, v, stream);
...
void self_attention(
    ...
    CHECK_ARGUMENT(
        query_elements == 0 || q != nullptr, "SelfAttention: query pointer must not be null.");

    CHECK_ARGUMENT(
        key_elements == 0 || k != nullptr, "SelfAttention: key pointer must not be null.");

    CHECK_ARGUMENT(
        value_elements == 0 || v != nullptr, "SelfAttention: value pointer must not be null.");

    if (output_elements == 0) { return; }

    const cudaStream_t cuda_stream = llaisys::device::nvidia::to_cuda_stream(stream);

#if LLAISYS_NVIDIA_SELF_ATTENTION_USE_CUDA_COMPAT
    ...
    // ========================================================
    // NVIDIA production implementation
    // ========================================================
    //
    // Preferred:
    //
