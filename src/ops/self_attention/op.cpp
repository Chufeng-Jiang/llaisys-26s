...
void self_attention(tensor_t attn_val, tensor_t q, tensor_t k, tensor_t v, float scale) {
  ...
  CHECK_ARGUMENT(attn_val->ndim() == 3, "SelfAttention: output tensor must be three-dimensional.");
  ...
  // of the KV cache.
  CHECK_ARGUMENT(total_len >= seqlen, "SelfAttention: total KV length must not be smaller than query sequence length.");

  // GQA/MQA requires every KV head to serve an equal number
  // of query heads.
  CHECK_ARGUMENT(nhead % nkvhead == 0, "SelfAttention: query head count must be divisible by KV head count.");
  ...
  CHECK_ARGUMENT(v->shape()[0] == total_len, "SelfAttention: key and value sequence lengths must match.");

  CHECK_ARGUMENT(v->shape()[1] == nkvhead, "SelfAttention: key and value head counts must match.");

  // ============================================================
  // Output shape checks
  // ============================================================

  CHECK_ARGUMENT(attn_val->shape()[0] == seqlen, "SelfAttention: output sequence length must match query sequence length.");

  CHECK_ARGUMENT(attn_val->shape()[1] == nhead, "SelfAttention: output head count must match query head count.");

  CHECK_ARGUMENT(attn_val->shape()[2] == dv, "SelfAttention: output head dimension must match value head dimension.");
  ...
  CHECK_ARGUMENT(attn_val->dtype() == q->dtype(), "SelfAttention: output and query must use the same data type.");

  CHECK_ARGUMENT(k->dtype() == q->dtype(), "SelfAttention: key and query must use the same data type.");

  CHECK_ARGUMENT(v->dtype() == q->dtype(), "SelfAttention: value and query must use the same data type.");

  // ============================================================
  // Device-type checks
  // ============================================================

  CHECK_ARGUMENT(attn_val->deviceType() == q->deviceType(), "SelfAttention: output and query must use the same device type.");

  CHECK_ARGUMENT(k->deviceType() == q->deviceType(), "SelfAttention: key and query must use the same device type.");

  CHECK_ARGUMENT(v->deviceType() == q->deviceType(), "SelfAttention: value and query must use the same device type.");

  // ============================================================
  // Device-ID checks
  // ============================================================

  CHECK_ARGUMENT(attn_val->deviceId() == q->deviceId(), "SelfAttention: output and query must be located on the same device.");

  CHECK_ARGUMENT(k->deviceId() == q->deviceId(), "SelfAttention: key and query must be located on the same device.");

  CHECK_ARGUMENT(v->deviceId() == q->deviceId(), "SelfAttention: value and query must be located on the same device.");
  ...
  switch (attn_val->deviceType()) {
    case LLAISYS_DEVICE_CPU:
      return cpu::self_attention(attn_val->data(), q->data(), k->data(), v->data(), scale, attn_val->dtype(), seqlen, nhead, dv, total_len, nkvhead, d);
    ...
    case LLAISYS_DEVICE_NVIDIA: {
      ...
      return nvidia::self_attention(attn_val->data(), q->data(), k->data(), v->data(), scale, attn_val->dtype(), seqlen, nhead, dv, total_len, nkvhead, d,
                                    runtime.stream());
    }
#endif
